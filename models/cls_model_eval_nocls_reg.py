...
# def wavelet_transform(img_list, level=1, wave='haar'):
#     # Convert list to tensor if needed
#     if isinstance(img_list, list):
#         img_list = [torch.tensor(img) if not isinstance(img, torch.Tensor) else img for img in img_list]
#         img = torch.stack(img_list)  # Stack into a single tensor
#     else:
...
#     coeffs = []
#     for i in range(3):  # Apply per-channel
#         c = pywt.wavedec2(img[:, i, :, :].detach().cpu().numpy(), wave, level=level)
#         coeffs.append([torch.tensor(comp).to(device) for comp in c])
#     return coeffs

# def wavelet_mse_loss(output_list, target_list, level=1, wave='haar'):
#     # Convert list inputs to tensors
#     output = wavelet_transform(output_list, level, wave)
#     target = wavelet_transform(target_list, level, wave)

#     loss = 0
#     for o, t in zip(output, target):
#         loss += torch.mean(torch.tensor([(c_o - c_t) ** 2 for c_o, c_t in zip(o, t)], dtype=torch.float32))
...
class ClsModel(YTMTNetBase):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            print('bhai optimzer mein aya hoon!!!!!')
            self.optimizer_G = torch.optim.Adam(self.net_i.parameters(),
                                                lr=opt.lr, betas=(0.9, 0.999), weight_decay=opt.wd)
    ...
    def get_loss(self, out_l, out_r):
        ...
        for i in range(self.opt.loss_col):
            ...
            if i != self.opt.loss_col -1:
                ...
                loss_rcnn_pixel = self.loss_dic['r_pixel'].get_loss(out_r_reflection, self.target_r) * 1.5 * self.opt.r_pixel_weight
                loss_icnn_vgg = self.loss_dic['t_vgg'].get_loss(out_r_clean, self.target_t) * self.opt.lambda_vgg
            # w= wavelet_mse_loss(out_r_clean, self.target_t, level=1, wave='haar') * 0.5
            # print('wave',self.loss_wavelet)
    ...
    def backward_G(self):
        ...
        # print('tar',self.target_t)
        # tar = tensor2im(self.target_t)
        # out = tensor2im(self.output_i)
        # self.loss_wavelet = wavelet_mse_loss(self.output_j[2*], self.target_t, level=1, wave='haar') * 0.5
        # self.loss_G += self.loss_wavelet
