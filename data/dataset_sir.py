...
class DSRTestDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        # print('bhai fn hai',fn)
        t_img = Image.open(f'/home/jatin/ReflectionRemoval/train_gt/{fn}').convert('RGB')
        m_img = Image.open(f'/home/jatin/ReflectionRemoval/train_inp/{fn}').convert('RGB')

        # target_width, target_height = 640, 480  # Define the target size

        # t_img = t_img.resize((target_width, target_height), Image.BICUBIC)
        # m_img = m_img.resize((target_width, target_height), Image.BICUBIC)

        # inp=cv2.imread(f'/home/jatin/ReflectionRemoval/train_inp/{fn}')
        # out=cv2.imread(f'/home/jatin/ReflectionRemoval/train_gt/{fn}')
        # if (inp.shape!=out.shape):
        #     print(f'fn{fn}')

        if self.if_align:
            # print('Bhai main idhar hoon!!!, self.align True hai')
